...
export default function Page({openMenu, handleOpenMenu}) {
    const menuItems = ["chats", "calls", "setting"]

    return (
        <aside className={`flex absolute top-0 left-0 z-40 w-screen h-dvh transition-transform ${openMenu ? "translate-x-0" : "-translate-x-full "} sm:translate-x-0 lg:relative lg:h-full lg:w-auto`}>
            <div className="h-full mr-2 px-3 py-4 bg-zinc-950 w-48 border-r border-zinc-800 flex flex-col justify-between lg:w-auto">
                ...
                <ul className="space-y-2 font-medium">
                    {menuItems.map((item, index) => (
                        <li key={index}>
                            <a href={item} className="linkMenu">
                                    ...
                                    <path strokeLinecap="round" strokeLinejoin="round"
                                          d={
                                              item === "calls" ? "M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                                                  :
                                                  item === "chats" ? "M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 0 1-.923 1.785A5.969 5.969 0 0 0 6 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337Z"
                                                      :
            ...
            <div className="h-full w-48 lg:hidden" onClick={handleOpenMenu}></div>
        </aside>
    )
}