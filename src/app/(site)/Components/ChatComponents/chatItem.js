...
export default function ChatItem({conversation,searchUsers }) {
    ...
    if(searchUsers)console.log(searchUsers)
                ...
                <li key={item.id}>
                    <div className="bg-zinc-800 p-2.5 rounded-lg flex items-center text-sm transition ease-linear duration-150 lg:hover:bg-zinc-950 lg:bg-transparent">
                        ...
                        <div className="flex-1 grid grid-rows-2 gap-1">
                            <span className="block font-bold">{item.name || item.users.filter(user=>user.username!==session?.user.name)[0].username}</span>
                        ...
                        <div className="mb-auto text-zinc-500 right-0 whitespace-nowrap">{formatAMPM(new Date(item.lastMessageAt))}</div>
                    </div>
                </li>))}
