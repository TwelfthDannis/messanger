...
const UpperPanel = ({conversation,user}) => {
    const otherUser=(conversation)?useOtherUser(conversation):null
    return (
        <div className="relative border-b border-zinc-800 h-14 bg-zinc-950 flex py-2 items-center rounded-t-xl">
            <div className="flex w-full justify-center items-center">
                ...
                <div className="grid grid-rows-2 items-center">
                    <span className="font-bold text-gray-300">{conversation?(conversation.name || otherUser.username):user.username}</span>
                    <span className="font-bold text-xs text-zinc-500">last active</span>
                </div>
            </div>
            {conversation?<button className="p-2 m-2 absolute right-0">
